...
class Client(Base):
    ...
    def to_aioauth_client(self) -> AioAuthClient:
        return AioAuthClient(
            ...
            grant_types=[GrantType(g) for g in self.grant_types.split(",")] if self.grant_types else [],
            response_types=[ResponseType(r) for r in self.response_types.split(",")] if self.response_types else [],
            scope=self.scope,
